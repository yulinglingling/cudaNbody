...
// GPU/CPU friendly 2D vector
class Vec2
{
public:
    float x, y;
    HD Vec2() : x(0), y(0) {}
    HD Vec2(float vx, float vy) : x(vx), y(vy) {}
    HD Vec2 operator+(const Vec2 &v) const { return {x + v.x, y + v.y}; }
    HD Vec2 operator-(const Vec2 &v) const { return {x - v.x, y - v.y}; }
    HD Vec2 operator*(float s) const { return {x * s, y * s}; }
    HD Vec2& operator+=(const Vec2 &v) { x += v.x; y += v.y; return *this; }
    HD float length() const { return sqrtf(x*x + y*y); }
    HD Vec2 normalize() const { float len = length(); return {x/len, y/len}; }
};

// GPU/CPU friendly Particle
class Particle
{
public:
    int id;
    ...
    Vec2 position;
    Vec2 velocity;
    // HD Particle() : id(4893), mass(1.0f), position(0,0), velocity(0,0) {}
};

// GPU-friendly computeForce
HD inline Vec2 computeForce(const Particle &target, const Particle &attractor, float cullRadius)
{
    Vec2 dir = attractor.position - target.position;
    float dist = dir.length();
    if(dist < 1e-3f) return {0.0f, 0.0f};
    dir = dir * (1.0f / dist);
    if(dist > cullRadius) return {0.0f, 0.0f};
...
// GPU-friendly updateParticle
HD inline Particle updateParticle(const Particle &pi, Vec2 force, float deltaTime)
{
    Particle result = pi;
    result.velocity += force * (deltaTime / pi.mass);
    result.position += result.velocity * deltaTime;
...
// CPU-only structs
struct StepParameters { float deltaTime=0.2f; float cullRadius=1.0f; };

class Pixel { public: unsigned char r,g,b,a; };
...
// CPU-side simulation interfaces
class AccelerationStructure
{
public:
    virtual void getParticles(std::vector<Particle> & particles, Vec2 position, float radius){}
